package com.hust.hui.quickmedia.common.emoticon;
...
public class EmotionWrapper {
    ...
    private int doDrawContent(Graphics2D g2d, String content, int y, int lineSize, int linePadding) {
        String[] strs = GraphicUtil.splitStr(content, lineSize, g2d.getFontMetrics());
        int index = 0;
        int x;
    ...
    public int doDrawVerticalContent(Graphics2D g2d,
        ...
        // 实际填充内容的高度， 需要排除上下间距
        String[] strs = GraphicUtil.splitVerticalStr(content, lineLen, fontMetrics);
    ...
    public static class Builder {
        ...
        public void calRealSize() {
            ...
            if (options.getDrawStyle() == ImgCreateOptions.DrawStyle.HORIZONTAL) {
                ...
                int lineNum = GraphicUtil.calLineNum(options.getContent(), realW - padding, fontMetrics);
                ...
                int lineNum = GraphicUtil.calVerticalLineNum(options.getContent(), realH - padding, fontMetrics);
                int contentSize = (fontSize + options.getLinePadding()) * lineNum + options.getLinePadding();
